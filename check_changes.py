...
if __name__ == "__main__":
    phone = contiguity.login(os.getenv('CONTIGUITY_KEY'))
    username = os.getenv('GRADES_USERNAME')
    password = os.getenv('GRADES_PASSWORD')
    if not username or not password or not phone:
        print("Error: Environment variables for username and/or password and/or api key are not set.")
        exit(1)
    ...
    if(len(changelog) != 0):
        try:
            ...
            pass
        print("Sending message: " + changelog)
